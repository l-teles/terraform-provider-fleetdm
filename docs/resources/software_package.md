...
- `package_path` (String) The filesystem path to the software package file. If set, the file will be uploaded to Fleet when its SHA256 differs from the current package. Supports .pkg, .msi, .exe, .zip, .deb, .rpm, .tar.gz and .ipa installer packages plus .sh, .py and .ps1 script installers. Mutually exclusive with package_s3.
