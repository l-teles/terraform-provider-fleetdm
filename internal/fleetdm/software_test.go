...
// TestClient_GetSoftwareInstaller_DecodesAllLabelScopes pins that the
// installer response decodes all three label scopes. labels_include_all was
// missing from SoftwareInstaller while its sibling response structs carried
// it, so an include-all package read back as having no targeting at all —
// which is also what made it impossible to verify an include-all round-trip.
func TestClient_GetSoftwareInstaller_DecodesAllLabelScopes(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if got := r.URL.Query().Get("team_id"); got != "3" {
			t.Errorf("team_id: got %q, want 3", got)
		}
		w.Header().Set("Content-Type", "application/json")
		_, _ = w.Write([]byte(`{"software_installer":{
			"software_title_id": 12,
			"name": "app.pkg",
			"labels_include_any": [{"id": 1, "name": "Workstations"}],
			"labels_exclude_any": [{"id": 2, "name": "Exempt"}],
			"labels_include_all": [{"id": 3, "name": "Engineering"}, {"id": 4, "name": "macOS"}]
		}}`))
	}))
	defer server.Close()
	...
	client, _ := NewClient(ClientConfig{ServerAddress: server.URL, APIKey: "test-api-key", VerifyTLS: false})
	teamID := 3
	installer, err := client.GetSoftwareInstaller(context.Background(), 12, &teamID)
	if err != nil {
		t.Fatalf("expected no error, got: %v", err)
	}

	names := func(labels []SoftwareLabel) []string {
	...
	for _, tc := range []struct {
		...
		{"labels_include_any", names(installer.LabelsIncludeAny), []string{"Workstations"}},
		{"labels_exclude_any", names(installer.LabelsExcludeAny), []string{"Exempt"}},
		{"labels_include_all", names(installer.LabelsIncludeAll), []string{"Engineering", "macOS"}},
	} {
		if !slices.Equal(tc.got, tc.want) {
			t.Errorf("%s: got %v, want %v", tc.scope, tc.got, tc.want)
		}
	}
}
